...
#define MAX_PAGE 8
#define TOPICS 6
const uint8_t topicStart[TOPICS] = { 0, 1, 2, 3, 7, 255 };

int8_t constantLastX;
uint16_t textErase;
uint16_t upgradeCost;
uint16_t downgradeCost;
bool performSave;
bool jumpSection;
...
{
	int16_t maxloc, loc, z;
	int8_t glowcol[60]; /* [1..60] */
	int8_t glowcolc[60]; /* [1..60] */
	uint16_t textloc[60]; /* [1..60] */
	uint8_t bank;
...
uint32_t JE_getCost( uint8_t itemType, uint16_t itemNum )
{
	...
	case 4:
		...
		const uint32_t port = itemType - 3,
...
{
	bool quit;
	uint8_t sel, screen, min = 0, max = 0;
	char *tempstr;
	char *tempstr2;
	bool mal_str = false;
	int len;
...
uint32_t JE_totalScore( const Player *this_player )
{
	uint32_t temp = this_player->cash;
...
int32_t JE_getValue( uint8_t itemType, uint16_t itemNum )
{
	...
	case 4:;
		...
		const uint32_t port = itemType - 3;
		const uint32_t item_power = player[twoPlayerMode ? port : 0].items.weapon[port].power - 1;
...
void JE_nextEpisode( void )
{
	...
	if (jumpBackToEpisode1)
	{
		...
		if ((mt_rand() % 6) == 0)
		{
			...
			for (uint32_t i = 0; i < COUNTOF(player[0].items.weapon); ++i)
...
void JE_initPlayerData( void )
{
	...
	for (uint32_t i = 0; i < COUNTOF(player[0].items.sidekick); ++i)
	...
	for (uint32_t p = 0; p < COUNTOF(player); ++p)
	{
		for (uint32_t i = 0; i < COUNTOF(player->items.weapon); ++i)
		{
	...
	do
	{
		if (episodeAvail[x-1])
		{
			...
			for (z = 0; z < 3; z++)
			{
				...
				JE_textShade(VGAScreen, 250, ((z+1) * 10) + 65 , difficultyNameB[temp5], 15, temp5 + ((uint8_t) (temp5 == 0)) - 1, FULL_SHADE);
			...
			for (z = 0; z < 3; z++)
			{
				...
				JE_textShade(VGAScreen, 250, ((z+1) * 10) + 125 , difficultyNameB[temp5], 15, temp5 + ((uint8_t) (temp5 == 0)) - 1, FULL_SHADE);
...
bool JE_inGameSetup( void )
{
	...
	const uint8_t help[6] /* [1..6] */ = {15, 15, 28, 29, 26, 27};
	uint8_t  sel;
	bool quit;
...
void adjust_difficulty( void )
{
	...
	const uint32_t score = twoPlayerMode ? (player[0].cash + player[1].cash) : JE_totalScore(&player[0]),
	            adjusted_score = roundf(score * score_multiplier[initialDifficulty]);

	uint32_t new_difficulty = 0;
...
void JE_SFCodes( uint8_t playerNum_, int16_t PX_, int16_t PY_, int16_t mouseX_, int16_t mouseY_ )
{
	...
	uint32_t ship = player[playerNum_-1].items.ship;
...
void JE_playCredits( void )
{
	...
	uint8_t currentpic = 0, fade = 0;
	int8_t fadechg = 1;
	uint8_t currentship = 0;
	int16_t shipx = 0, shipxwait = 0;
	int8_t shipxc = 0, shipxca = 0;
	...
	for (int ticks = 0; ticks < ticks_max; ++ticks)
	{
		...
		uint32_t ship_sprite = ships[currentship].shipgraphic;
		if (shipxc < -10)
...
void JE_endLevelAni( void )
{
	...
	if (!constantPlay)
	{
		...
		{
			// front, rear
			for (uint32_t i = 0; i < COUNTOF(player[p].items.weapon); ++i)
				saveTemp[SAVE_FILES_SIZE + player[p].items.weapon[i].id] = 1;

			// options
			for (uint32_t i = 0; i < COUNTOF(player[p].items.sidekick); ++i)
	...
	if (twoPlayerMode)
	{
		for (uint32_t i = 0; i < 2; ++i)
		{
			snprintf(tempStr, sizeof(tempStr), "%s %u", miscText[40 + i], player[i].cash);
...
void JE_inGameDisplays( void )
{
	...
	for (uint32_t i = 0; i < ((twoPlayerMode && !galagaMode) ? 2 : 1); ++i)
	{
		snprintf(tempstr, sizeof(tempstr), "%u", player[i].cash);
	...
	if (onePlayerAction || twoPlayerMode)
	{
		...
		{
			const uint32_t extra_lives = *player[temp].lives - 1;
			...
			{
				for (uint32_t i = 0; i < extra_lives; ++i)
				{
					blit_sprite2(VGAScreen, tempW, temp5, shapes9, 285);
	...
	for (uint32_t i = 0; i < COUNTOF(player); ++i)
	{
		...
		for (uint32_t j = player[i].superbombs; j > 0; --j)
		{
...
void JE_playerMovement( Player *this_player,
                        ...
                        uint16_t *mouseX_, uint16_t *mouseY_ )
{
	int16_t mouseXC, mouseYC;
	int16_t accelXC, accelYC;
...
redo:
	...
	if (this_player->is_alive && !endLevel)
	{
		if (!twoPlayerLinked || playerNum_ < 2)
		{
			...
			{
				for (uint32_t i = 1; i < COUNTOF(player->old_x); ++i)
				{
		...
		if (this_player->is_alive)
		{
			if (!endLevel)
			{
				...
				if (!(twoPlayerLinked && playerNum_ == 2))
				{
					...
					{
						const uint32_t item = this_player->items.weapon[temp].id;
						...
						if (item > 0)
						{
							...
							{
								const uint32_t item_power = galagaMode ? 0 : this_player->items.weapon[temp].power - 1,
				...
				{
					for (uint32_t i = 0; i < COUNTOF(player->items.sidekick); ++i)
					{
						uint32_t shot_i = (i == 0) ? SHOT_LEFT_SIDEKICK : SHOT_RIGHT_SIDEKICK;
	...
	if ((playerNum_ == 2 || !twoPlayerMode) && !endLevel)
	{
		for (uint32_t i = 0; i < COUNTOF(this_player->sidekick); ++i)
		{
			...
			if (this_option->option > 0)
			{
				...
				const uint32_t sprite = this_option->gr[this_player->sidekick[i].animation_frame] + this_player->sidekick[i].charge;
...
void JE_playerCollide( Player *this_player, uint8_t playerNum_ )
{
	char tempStr[256];
				...
				if (tempI4 > 29999)
				{
					...
					else if (tempI4 > 32000)
					{
						if (playerNum_ == 2)
						{
							...
							uint32_t temp = player[1].items.sidekick_level - 100 - 1;
							for (uint32_t i = 0; i < COUNTOF(player[1].items.sidekick); ++i)
