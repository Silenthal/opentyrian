...
uint16_t levelEnemyMax;
uint16_t levelEnemyFrequency;
uint16_t levelEnemy[40]; /* [1..40] */

char tempStr[31];

/* Data used for ItemScreen procedure to indicate items available */
uint8_t itemAvail[9][10]; /* [1..9, 1..10] */
uint8_t itemAvailMax[9]; /* [1..9] */
...
start_level_first:
	...
	for (uint32_t i = 0; i < COUNTOF(player); ++i)
	{
		for (uint32_t j = 0; j < COUNTOF(player->old_x); ++j)
		{
	...
	uint32_t old_weapon_bar[2] = { 0, 0 };  // only redrawn when they change
	...
	for (uint32_t i = 0; i < COUNTOF(enemyShotAvail); i++)
		enemyShotAvail[i] = 1;
...
level_loop:
	...
	if (!endLevel) // draw HUD
	{
		...
		for (uint32_t i = 0; i < 2; ++i)
		{
			uint32_t item_power = player[twoPlayerMode ? i : 0].items.weapon[i].power;
			...
			if (old_weapon_bar[i] != item_power)
			{
				...
				for (uint32_t j = 1; j <= item_power; ++j)
				{
...
draw_player_shot_loop_end:
	...
	for (uint32_t i = 0; i < (twoPlayerMode ? 2 : 1); ++i)
	...
	if (debug)
	{
		...
		debugHist = debugHist + abs((int32_t)debugTime - (int32_t)lastDebugTime);
		debugHistCount++;
	...
	{
		displayTime--;
		JE_outTextAndDarken(VGAScreen, 90, 10, miscText[59], 15, (uint8_t)flash - 8, FONT_SHAPES);
		flash += flashChange;
		if (flash > 4 || flash == 0)
...
void JE_loadMap( void )
{
	...
	uint16_t x, y;
	int16_t yy;
	uint16_t mapSh[3][128]; /* [1..3, 0..127] */
	uint8_t *ref[3][128]; /* [1..3, 0..127] */
	char s[256];

	uint8_t mapBuf[15 * 600]; /* [1..15 * 600] */
	uint16_t bufLoc;
...
new_game:
			...
			do
			{
				...
				if (s[0] == ']')
				{
					...
					case 'Q':
						...
						{
							for (uint32_t i = 0; i < 2; ++i)
								snprintf(levelWarningText[i], sizeof(*levelWarningText), "%s %u", miscText[40], player[i].cash);
	...
	efread(&mapX,  sizeof(uint16_t), 1, level_f);
	efread(&mapX2, sizeof(uint16_t), 1, level_f);
	efread(&mapX3, sizeof(uint16_t), 1, level_f);

	efread(&levelEnemyMax, sizeof(uint16_t), 1, level_f);
	for (x = 0; x < levelEnemyMax; x++)
	{
		efread(&levelEnemy[x], sizeof(uint16_t), 1, level_f);
	}

	efread(&maxEvent, sizeof(uint16_t), 1, level_f);
	for (x = 0; x < maxEvent; x++)
	{
		efread(&eventRec[x].eventtime, sizeof(uint16_t), 1, level_f);
		efread(&eventRec[x].eventtype, sizeof(uint8_t), 1, level_f);
		efread(&eventRec[x].eventdat,  sizeof(int16_t), 1, level_f);
		efread(&eventRec[x].eventdat2, sizeof(int16_t), 1, level_f);
		efread(&eventRec[x].eventdat3, sizeof(int8_t), 1, level_f);
		efread(&eventRec[x].eventdat5, sizeof(int8_t), 1, level_f);
		efread(&eventRec[x].eventdat6, sizeof(int8_t), 1, level_f);
		efread(&eventRec[x].eventdat4, sizeof(uint8_t), 1, level_f);
	}
	...
	efread(mapSh, sizeof(uint16_t), sizeof(mapSh) / sizeof(uint16_t), level_f);
	for (temp = 0; temp < 3; temp++)
	{
	...
	for (int z = 0; z < 600; z++)
	{
		...
		if (shapeBlank)
			...
			efread(shape, sizeof(uint8_t), sizeof(shape), shpFile);
		...
		for (int x = 0; x <= 71; ++x)
		{
			if (mapSh[0][x] == z+1)
			{
				...
				ref[0][x] = (uint8_t *)megaData1.shapes[x].sh;
			}
		}
		...
		for (int x = 0; x <= 71; ++x)
		{
			if (mapSh[1][x] == z+1)
			{
				if (x != 71 && !shapeBlank)
				{
					...
					ref[1][x] = (uint8_t *)megaData2.shapes[x].sh;
				}
				else
				{
					ref[1][x] = NULL;
		...
		for (int x = 0; x <= 71; ++x)
		{
			if (mapSh[2][x] == z+1)
			{
				if (x < 70 && !shapeBlank)
				{
					...
					ref[2][x] = (uint8_t *)megaData3.shapes[x].sh;
				}
				else
				{
					ref[2][x] = NULL;
	...
	efread(mapBuf, sizeof(uint8_t), 14 * 300, level_f);
	...
	efread(mapBuf, sizeof(uint8_t), 14 * 600, level_f);
	...
	efread(mapBuf, sizeof(uint8_t), 15 * 600, level_f);
...
bool JE_titleScreen( bool animate )
{
	...
	uint16_t waitForDemo;
	uint8_t menu = 0;
	bool redraw = true,
	           fadeIn = false;

	uint16_t temp; /* uint8_t temp; from varz.h will overflow in for loop */
				...
				case SDLK_RETURN:
					...
					case 0: /* New game */
						...
						if (select_gameplay() && select_episode() && select_difficulty())
						{
							...
							{
								uint32_t initial_cash[] = { 10000, 15000, 20000, 30000, 35000 };
...
bool JE_searchFor/*enemy*/( uint8_t PLType )
{
	bool tempb = false;
	uint8_t temp;
...
void JE_eventSystem( void )
{
	...
	case 33: /* Enemy From other Enemies */
		if (!((eventRec[eventLoc-1].eventdat == 512 || eventRec[eventLoc-1].eventdat == 513) && (twoPlayerMode || onePlayerAction || superTyrian)))
		{
			...
			{
				const uint32_t lives = *player[0].lives;
	...
	case 71:
		if (((((intptr_t)mapYPos - (intptr_t)&megaData1.mainmap) / sizeof(uint8_t *)) * 2) <= (unsigned)eventRec[eventLoc-1].eventdat2)
		{
			JE_eventJump(eventRec[eventLoc-1].eventdat);
		}
		break;
