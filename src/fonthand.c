...
int16_t defaultBrightness = -3;
uint8_t textGlowFont, textGlowBrightness = 6;

bool levelWarningDisplay;
uint8_t levelWarningLines;
char levelWarningText[10][61]; /* [1..10] of string [60] */
bool warningRed;

uint8_t warningSoundDelay;
uint16_t armorShipDelay;
uint8_t warningCol;
int8_t warningColChange;

void JE_dString( SDL_Surface * screen, int x, int y, const char *s, uint32_t font )
{
	int bright = 0;
...
int JE_textWidth( const char *s, uint32_t font )
{
	int x = 0;
...
void JE_textShade( SDL_Surface * screen, int x, int y, const char *s, uint32_t colorbank, int brightness, uint32_t shadetype )
{
	switch (shadetype)
	{
		case PART_SHADE:
...
void JE_outText( SDL_Surface * screen, int x, int y, const char *s, uint32_t colorbank, int brightness )
{
	int bright = 0;
...
void JE_outTextModify( SDL_Surface * screen, int x, int y, const char *s, uint32_t filter, uint32_t brightness, uint32_t font )
{
	for (int i = 0; s[i] != '\0'; ++i)
	{
		int sprite_id = font_ascii[(uint8_t)s[i]];
