...
void poll_joystick( int j )
{
	...
	for (uint32_t d = 0; d < COUNTOF(joystick[j].direction); d++)
	{
	...
	// update action state
	for (uint32_t d = 0; d < COUNTOF(joystick[j].action); d++)
	{
...
void push_joysticks_as_keyboard( void )
{
	...
	for (int j = 0; j < joysticks; j++)
	{
		...
		for (uint32_t d = 0; d < COUNTOF(joystick[j].direction_pressed); d++)
		{
...
void reset_joystick_assignments( int j )
{
	...
	for (uint32_t a = 0; a < COUNTOF(joystick[j].assignment); a++)
	{
		// clear assignments
		for (uint32_t i = 0; i < COUNTOF(joystick[j].assignment[a]); i++)
...
bool load_joystick_assignments( int j )
{
	...
	if ((setting = cJSON_GetObjectItem(config, "assignments")))
	{
		for (uint32_t i = 0; i < COUNTOF(joystick->assignment); ++i)
		{
			...
			for (uint32_t k = 0; k < COUNTOF(*joystick->assignment); ++k)
			{
...
bool detect_joystick_assignment( int j, Joystick_assignment *assignment )
{
	...
	do
	{
		...
		for (int i = 0; i < buttons; ++i)
		{
			uint8_t new_button = SDL_JoystickGetButton(joystick[j].handle, i),
		...
		for (int i = 0; i < hats; ++i)
		{
			uint8_t new_hat = SDL_JoystickGetHat(joystick[j].handle, i),
