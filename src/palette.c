...
static uint32_t rgb_to_yuv( int r, int g, int b );
...
uint32_t rgb_palette[256], yuv_palette[256];
...
void set_palette( Palette colors, uint32_t first_color, uint32_t last_color )
{
	SDL_Surface *const surface = SDL_GetVideoSurface();
	const uint32_t bpp = surface->format->BitsPerPixel;
	
	for (uint32_t i = first_color; i <= last_color; ++i)
	{
		palette[i] = colors[i];
...
void set_colors( SDL_Color color, uint32_t first_color, uint32_t last_color )
{
	SDL_Surface *const surface = SDL_GetVideoSurface();
	const uint32_t bpp = surface->format->BitsPerPixel;
...
void init_step_fade_palette( int diff[256][3], Palette colors, uint32_t first_color, uint32_t last_color )
{
	for (uint32_t i = first_color; i <= last_color; i++)
	{
...
void init_step_fade_solid( int diff[256][3], SDL_Color color, uint32_t first_color, uint32_t last_color )
{
	for (uint32_t i = first_color; i <= last_color; i++)
	{
...
void step_fade_palette( int diff[256][3], int steps, uint32_t first_color, uint32_t last_color )
{
	assert(steps > 0);
...
void fade_palette( Palette colors, int steps, uint32_t first_color, uint32_t last_color )
{
	assert(steps > 0);
...
void fade_solid( SDL_Color color, int steps, uint32_t first_color, uint32_t last_color )
{
	assert(steps > 0);
