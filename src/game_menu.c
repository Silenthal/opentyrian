...
static int16_t lastDirection;
static bool firstMenu9, paletteChanged;
static JE_MenuChoiceType menuChoices;
static int16_t col, colC;
static uint8_t lastCurSel;
static int16_t curMenu;
static uint8_t curSel[MENU_MAX]; /* [1..maxmenu] */
static uint8_t curItemType, curItem, cursor;
static bool leftPower, rightPower, rightPowerAfford;
static uint8_t currentCube;
static bool keyboardUsed;

static uint8_t planetAni, planetAniWait;
static uint8_t currentDotNum, currentDotWait;
static float navX, navY, newNavX, newNavY;
static int16_t tempNavX, tempNavY;
static uint8_t planetDots[5]; /* [1..5] */
static int16_t planetDotX[5][10], planetDotY[5][10]; /* [1..5, 1..10] */
...
static const uint8_t menuEsc[MENU_MAX] = { 0, 1, 1, 1, 2, 3, 3, 1, 8, 0, 0, 11, 3, 0 };
static const uint8_t itemAvailMap[7] = { 1, 2, 3, 9, 4, 6, 7 };
static const uint16_t planetX[21] = { 200, 150, 240, 300, 270, 280, 320, 260, 220, 150, 160, 210, 80, 240, 220, 180, 310, 330, 150, 240, 200 };
static const uint16_t planetY[21] = {  40,  90,  90,  80, 170,  30,  50, 130, 120, 150, 220, 200, 80,  50, 160,  10,  55,  55,  90,  90,  40 };
static const uint32_t cube_line_chars = sizeof(*cube->text) - 1;
static const uint32_t cube_line_width = 150;


/*** Functions ***/
static uint32_t *playeritem_map( PlayerItems *items, uint32_t i )
{
	uint32_t * const map[] = { &items->ship, &items->weapon[FRONT_WEAPON].id, &items->weapon[REAR_WEAPON].id, &items->shield, &items->generator, &items->sidekick[LEFT_SIDEKICK], &items->sidekick[RIGHT_SIDEKICK] };
...
{
	int32_t tempL = player[0].cash;
	uint16_t itemNum = *playeritem_map(&player[0].items, curSel[1] - 2);
	...
	case 3:
	case 4:
		for (uint32_t i = 1; i < player[0].items.weapon[curSel[1]-3].power; ++i)
		{
	...
	*/
	...
	do
	{
		...
		if (curMenu == 1 && (curSel[curMenu] == 3 || curSel[curMenu] == 4))
		{
			...
			const uint32_t item       = player[0].items.weapon[curSel[1] - 3].id,
		...
		if (curMenu == 12)
		{
			...
			for (uint32_t i = 0; i < COUNTOF(menu_item); i++)
			{
		...
		if (curMenu == 4)
		{
			...
			{
				const uint32_t port = curSel[1] - 3,  // 0 or 1 (front or back)
		...
		if (((curMenu <= 2 || curMenu == 5 || curMenu == 6 || curMenu >= 10) && !twoPlayerMode) || (curMenu == 4 && (curSel[1] >= 1 && curSel[1] <= 6)))
		{
			if (curMenu != 4)
			{
				...
				snprintf(buf, sizeof buf, "%u", player[0].cash);
		...
		if (((curMenu >= 0 && curMenu <= 2) || curMenu == 5 || curMenu == 6 || curMenu >= 9) || (curMenu == 4 && (curSel[1] == 2 || curSel[1] == 5)))
		{
			if (twoPlayerMode)
			{
				...
				{
					snprintf(buf, sizeof(buf), "%s %u", miscText[40 + i], player[i].cash);
		...
		if (curMenu == 9)
		{
			for (uint32_t i = 0; i < COUNTOF(inputDevice); i++)
			{
		...
		if (mouseButton > 0)
		{
			...
			{
				const uint8_t mouseSelectionY[MENU_MAX] = { 16, 16, 16, 16, 26, 12, 11, 28, 0, 16, 16, 16, 8, 16 };
...
bool load_cube( int cube_slot, int cube_index )
{
	...
	uint32_t line = 0, line_chars = 0, line_width = 0;
	...
	for (; ; )
	{
		...
		for (uint32_t i = 0; ; ++i)
		{
			...
			if (end_of_word)
			{
				...
				uint32_t word_chars = strlen(word),
...
void JE_scaleBitmap( SDL_Surface *dst_bitmap, const SDL_Surface *src_bitmap,  int x1, int y1, int x2, int y2 )
{
	...
	uint8_t *dst = dst_bitmap->pixels;  /* 8-bit specific */
	const uint8_t *src, *src_w;  /* 8-bit specific */
	...
	for (int i = 0; i < h; i++)
	{
		...
		src = src_w = (uint8_t *)(src_bitmap->pixels) + (src_bitmap->w * ((unsigned int)cumulative_skip_h));
	...
	case 5: /* keyboard settings */
		...
		else /* change key */
		{
			...
			if (newkey)
			{
				...
				for (uint32_t i = 0; i < COUNTOF(keySettings); ++i)
				{
	...
	case 12: //joy
		...
		default:
			...
			if (detect_joystick_assignment(joystick_config, &temp))
			{
				...
				for (uint32_t i = 0; i < COUNTOF(*joystick->assignment); i++)
				{
...
void JE_weaponViewFrame( void )
{
	...
	for (uint32_t i = 0; i < 2; ++i)
	{
		...
		{
			const uint32_t item       = player[0].items.weapon[i].id,
	...
	if (options[player[0].items.sidekick[LEFT_SIDEKICK]].wport > 0)
	{
		...
		{
			const uint32_t item = player[0].items.sidekick[LEFT_SIDEKICK];
	...
	if (options[player[0].items.sidekick[RIGHT_SIDEKICK]].wport > 0)
	{
		...
		{
			const uint32_t item = player[0].items.sidekick[RIGHT_SIDEKICK];
