...
uint16_t network_player_port = NET_PORT,
...
static uint16_t last_out_sync = 0, queue_in_sync = 0, queue_out_sync = 0, last_ack_sync = 0;
static uint32_t last_in_tick = 0, last_out_tick = 0;
...
static uint16_t last_state_in_sync = 0, last_state_out_sync = 0;
static uint32_t last_state_in_tick = 0;

static bool net_initialized = false;
static bool connected = false, quit = false;
#endif

uint32_t thisPlayerNum = 0;  /* Player number on this PC (1 or 2) */
...
void network_prepare( uint16_t type )
{
...
bool network_send( int len )
{
	...
	uint16_t i = last_out_sync - queue_out_sync;
...
static int network_acknowledge( uint16_t sync )
{
			...
			if (packet_temp->channel == 0 && packet_temp->len >= 4)
			{
				switch (SDLNet_Read16(&packet_temp->data[0]))
				{
					case PACKET_ACKNOWLEDGE:
						if ((uint16_t)(SDLNet_Read16(&packet_temp->data[2]) - last_ack_sync) < NET_PACKET_QUEUE)
						{
							last_ack_sync = SDLNet_Read16(&packet_temp->data[2]);
						}

						{
							uint16_t i = SDLNet_Read16(&packet_temp->data[2]) - queue_out_sync;
							if (i < NET_PACKET_QUEUE)
							{
								if (packet_out[i])
								{
						...
						while (packet_out[0] == NULL && (uint16_t)(last_ack_sync - queue_out_sync) < NET_PACKET_QUEUE)
						{
					...
					case PACKET_GAME_MENU:
						{
							uint16_t i = SDLNet_Read16(&packet_temp->data[2]) - queue_in_sync;
							if (i < NET_PACKET_QUEUE)
							{
								if (packet_in[i] == NULL)
					...
					case PACKET_STATE:
						...
						{
							uint16_t i = SDLNet_Read16(&packet_temp->data[2]) - last_state_in_sync + 1;
							if (i < NET_PACKET_QUEUE)
							{
					...
					case PACKET_STATE_RESEND:
						...
						{
							uint16_t i = last_state_out_sync - SDLNet_Read16(&packet_temp->data[2]);
...
int network_connect( void )
{
	...
	uint16_t episodes = 0, episodes_local = 0;
