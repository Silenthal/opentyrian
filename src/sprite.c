...
void load_sprites( unsigned int table, FILE *f )
{
	...
	for (unsigned int i = 0; i < sprite_table[table].count; ++i)
	{
		...
		efread(&cur_sprite->width,  sizeof(uint16_t), 1, f);
		efread(&cur_sprite->height, sizeof(uint16_t), 1, f);
		efread(&cur_sprite->size,   sizeof(uint16_t), 1, f);
		
		cur_sprite->data = malloc(cur_sprite->size);
		
		efread(cur_sprite->data, sizeof(uint8_t), cur_sprite->size, f);
	}
}
...
void blit_sprite( SDL_Surface *surface, int x, int y, unsigned int table, unsigned int index )
{
	...
	const uint8_t *data = cur_sprite->data;
	const uint8_t * const data_ul = data + cur_sprite->size;
	...
	const uint8_t * const pixels_ll = (uint8_t *)surface->pixels,  // lower limit
	            * const pixels_ul = (uint8_t *)surface->pixels + (surface->h * surface->pitch);  // upper limit
...
void blit_sprite_hv_unsafe( SDL_Surface *surface, int x, int y, unsigned int table, unsigned int index, uint8_t hue, int8_t value )
{
	if (index >= sprite_table[table].count || !sprite_exists(table, index))
	{
		assert(false);
...
// does not clip on left or right edges of surface
void blit_sprite_hv( SDL_Surface *surface, int x, int y, unsigned int table, unsigned int index, uint8_t hue, int8_t value )
{
	if (index >= sprite_table[table].count || !sprite_exists(table, index))
	{
		assert(false);
	...
	for (; data < data_ul; ++data)
	{
		...
		default:  // set a pixel
			...
			if (pixels >= pixels_ll)
			{
				uint8_t temp_value = (*data & 0x0f) + value;
				if (temp_value > 0xf)
...
// does not clip on left or right edges of surface
void blit_sprite_hv_blend( SDL_Surface *surface, int x, int y, unsigned int table, unsigned int index, uint8_t hue, int8_t value )
{
	if (index >= sprite_table[table].count || !sprite_exists(table, index))
	{
		assert(false);
...
void JE_loadCompShapesB( Sprite2_array *sprite2s, FILE *f )
{
	...
	sprite2s->data = malloc(sizeof(uint8_t) * sprite2s->size);
	efread(sprite2s->data, sizeof(uint8_t), sprite2s->size, f);
}
...
void blit_sprite2( SDL_Surface *surface, int x, int y, Sprite2_array sprite2s, unsigned int index )
{
	...
	const uint8_t *data = sprite2s.data + SDL_SwapLE16(((uint16_t *)sprite2s.data)[index - 1]);
...
// does not clip on left or right edges of surface
void blit_sprite2_filter( SDL_Surface *surface, int x, int y, Sprite2_array sprite2s, unsigned int index, uint8_t filter )
{
	assert(surface->format->BitsPerPixel == 8);
	uint8_t *             pixels =    (uint8_t *)surface->pixels + (y * surface->pitch) + x;
	const uint8_t * const pixels_ll = (uint8_t *)surface->pixels,  // lower limit
...
void JE_loadMainShapeTables( const char *shpfile )
{
	...
	uint16_t shpNumb;
	int32_t shpPos[SHP_NUM + 1]; // +1 for storing file length
	
	efread(&shpNumb, sizeof(uint16_t), 1, f);
	assert(shpNumb + 1u == COUNTOF(shpPos));
	
	for (unsigned int i = 0; i < shpNumb; ++i)
		efread(&shpPos[i], sizeof(int32_t), 1, f);
...
{
	for (uint32_t i = 0; i < COUNTOF(sprite_table); ++i)
		free_sprites(i);
