...
{
	uint32_t nlps;            /* Number of 'pages', max 256. */
	uint32_t nRecords;        /* Number of 'records', max 65535 */
} anim_FileHeader_t;
typedef struct anim_LargePageHeader_s
{
	uint32_t baseRecord;      /* The first record's number */
	uint32_t nRecords;        /* Number of records.  Supposedly there are bit flags but I saw no such code */
	uint32_t nBytes;	      /* Number of bytes used, excluding headers */
} anim_LargePageHeader_t;


/*** Globals ***/
uint8_t CurrentPageBuffer[65536];
anim_LargePageHeader_t PageHeader[256];
uint32_t CurrentPageRecordSizes[256];
...
/*** Function decs ***/
int JE_playRunSkipDump( uint8_t *, uint32_t );
void JE_closeAnim( void );
int JE_loadAnim( const char * );
int JE_renderFrame( uint32_t );
int JE_findPage ( uint32_t );
int JE_drawFrame( uint32_t );
int JE_loadPage( uint32_t );
...
int JE_drawFrame( uint32_t framenumber )
{
	int ret;
...
int JE_findPage( uint32_t framenumber )
{
	uint32_t i;
...
int JE_renderFrame( uint32_t framenumber )
{
	uint32_t i, offset, destframe;
 ...
 */
int JE_playRunSkipDump( uint8_t *incomingBuffer, uint32_t IncomingBufferLength )
{
	...
	while (1)
	{
		...
		uint32_t opcode = MSG_ReadByte(pBuffer_IN);
		...
		if (opcode == ANI_LONG_OP) /* long ops */
		{
			...
			else /* Now things get a bit more interesting... */
			{
				...
				{
					uint32_t count = opcode & ~ANI_LONG_RLE; /* Clear flag */

					/* Extract another byte */
					uint32_t value = MSG_ReadByte(pBuffer_IN);
		...
		else /* short ops */
		{
			...
			{
				uint32_t count = opcode & ~ANI_SHORT_SKIP; /* clear flag to get count */
				SZ_Seek(pBuffer_OUT, count, SEEK_CUR);
			}
			else if (opcode == ANI_SHORT_RLE) /* Short RLE, memset the destination */
			{
				/* Extract a few more bytes */
				uint32_t count = MSG_ReadByte(pBuffer_IN);
				uint32_t value = MSG_ReadByte(pBuffer_IN);
