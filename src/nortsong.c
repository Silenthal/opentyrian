...
uint16_t frameCount, frameCount2, frameCountMax;

uint8_t *digiFx[SAMPLE_COUNT] = { NULL }; /* [1..soundnum + 9] */
uint16_t fxSize[SAMPLE_COUNT]; /* [1..soundnum + 9] */

uint16_t tyrMusicVolume, fxVolume;
uint16_t fxPlayVol;
uint16_t tempVolume;

uint16_t speed; /* JE: holds timer speed for 70Hz */
...
{
	int32_t delay = target - SDL_GetTicks();
	if (delay > 0)
		SDL_Delay(delay);
}
...
void JE_loadSndFile( const char *effects_sndfile, const char *voices_sndfile )
{
	...
	uint16_t x;
	int32_t templ;
	int32_t sndPos[2][SAMPLE_COUNT + 1];
	uint16_t sndNum;
